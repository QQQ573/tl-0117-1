...
import { examCenters, busRoutes } from '@/mock/data'
...
const centerOptions = examCenters.map((c) => ({ label: c.name, value: c.id }))
const vehicleOptions = busRoutes.map((r) => ({ label: `${r.vehicleNo} (${r.id})`, value: r.id }))

const centerSelectAll = computed({
  get: () => store.selectedCenters.length === examCenters.length,
  set: (v: boolean) => store.setCenters(v ? examCenters.map((c) => c.id) : []),
})

const vehicleSelectAll = computed({
  get: () => store.selectedVehicles.length === busRoutes.length,
  set: (v: boolean) => store.setVehicles(v ? busRoutes.map((r) => r.id) : []),
})
...
<template>
  <div class="flex flex-wrap items-center gap-3 px-5 py-3 bg-[#0f172a] border-b border-[#1e293b]">
    <div class="relative group">
      ...
      >
        考点 ({{ store.selectedCenters.length }}/{{ examCenters.length }})
    ...
    <div class="relative group">
      <button
        class="px-3 py-1.5 rounded bg-[#1e293b] text-[#cbd5e1] text-xs font-mono hover:bg-[#334155] transition-colors flex items-center gap-1"
      >
        车辆 ({{ store.selectedVehicles.length }}/23)
        <span class="text-[10px]">▾</span>
      </button>
    ...
    <div v-if="isReadOnly" class="ml-auto px-3 py-1.5 rounded bg-[#ffa726]/15 text-[#ffa726] text-xs font-mono border border-[#ffa726]/30">
      只读模式 (iPad)
    </div>
  </div>
</template>
